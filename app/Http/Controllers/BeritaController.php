...
class BeritaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('foto')) {
            ...
            // Generate nama file unik dengan timestamp + original name
            $filename = time() . '_' . $file->getClientOriginalName();
            // Simpan file ke direktori 'berita_images' di dalam disk 'public'
            // Path yang dikembalikan adalah path relatif dari root disk 'public' (misal: berita_images/nama_file.jpg)
            $path = $file->storeAs('berita_images', $filename, 'public'); // Menggunakan 'berita_images' dan storeAs

            // Simpan path relatif ini ke dalam data yang akan disimpan ke database
        ...
        // 3. Tentukan penulis secara otomatis dari nama user yang sedang login
        $validatedData['penulis'] = Auth::user()->name;

        // 4. Tentukan tanggal publikasi jika tidak diisi dari form
        if (empty($validatedData['tanggal_publikasi'])) {
            $validatedData['tanggal_publikasi'] = now();
        } else {
            // Pastikan format tanggal sesuai untuk kolom datetime di DB
            $validatedData['tanggal_publikasi'] = \Carbon\Carbon::parse($validatedData['tanggal_publikasi']);
        }

        // 5. Buat record berita baru di database
    ...
    public function update(Request $request, Berita $berita)
    {
        ...
        if ($request->hasFile('foto')) {
            // Hapus foto lama dari storage jika ada dan file tersebut benar-benar ada
            ...
            $file = $request->file('foto');
            $filename = time() . '_' . $file->getClientOriginalName();
            // Simpan foto baru ke dalam disk 'public'
            $path = $file->storeAs('berita_images', $filename, 'public'); // Menggunakan 'berita_images' dan storeAs

            // Tambahkan path relatif file baru ke data yang akan di-update
            $validatedData['foto'] = $path;
            Log::info("Foto baru diunggah dan disimpan saat update: " . $path);
        } else {
            // Pertahankan foto lama jika tidak ada foto baru diunggah
